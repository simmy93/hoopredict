...
// PlayerCard component removed - not used anymore
...
export default function ManageNew({
    ...
    useEffect(() => {
        ...
        if (startersFromDb.length > 0) {
            ...
            startersFromDb.forEach((player) => {
                // Map by lineup_position: positions 1-5 map to array indices 0-4
    ...
    const handleLineupTypeChange = (type: string) => {
        ...
        // Only adjust lineup if actually changing the type (not on initial load)
        if (oldType && oldType !== newType) {
            const newConfig = LINEUP_CONFIGS[newType]
            const newStarters: (FantasyTeamPlayer | null)[] = [null, null, null, null, null]
            const playersToReturn: FantasyTeamPlayer[] = []

            // Separate current starters by position
    ...
    const getCourtPosition = (slotIndex: number, config: LineupConfig): { left: string; top: string } => {
        ...
        // Use full image height from 15% (top) to 85% (bottom)

        if (slotIndex < config.guards) {
            // Guards - bottom of court (85% from top)
        ...
        if (slotIndex < config.guards + config.forwards) {
            // Forwards - middle of court (50% from top)
        ...
        // Centers - top of court (15% from top)
    ...
    // Point percentages removed - multipliers applied on backend after each round
